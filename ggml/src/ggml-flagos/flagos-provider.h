...
enum class flagos_provider_kind : uint32_t {
    ...
    ai_accelerator = 1,
    cpu_accelerator = 2,
    // Keep the original values stable.  These additional kinds let a new
    // chip expose a DSP, matrix engine, or deliberately generic accelerator
    // without pretending it is an NPU or GPU in the common registry.
    dsp = 3,
    matrix = 4,
    other = 5,
};
