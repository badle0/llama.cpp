...
def main():
    ...
    target = GPUTarget("hip", args.arch, 32) if args.arch else \
    ...
    kernels = [
        ...
        (flagos_add_rms_norm_mul_residual_f32, {
            "residual_output": "*fp32", "mul_output": "*fp32", "x": "*fp32", "bias": "*fp32", "weight": "*fp32",
            "n_cols": "i32", "eps": "fp32", "BLOCK": "constexpr",
        }, "flagos_add_rms_norm_mul_residual_f32"),
    ...
    available = {item[2] for item in kernels}
    missing = set(args.kernel) - available
    if missing:
        raise RuntimeError(
            "requested AMD smoke kernels are unknown: " + ", ".join(sorted(missing)))
    ...
    args.output_dir = Path(temporary_output.name) / "package"
    ...
    for item in kernels:
        ...
        entries.append({
            ...
            "argument_count": sum(value != "constexpr" for value in signature.values()),
        ...
        validate_hsaco(output, target.arch, entries[-1])

    if not entries:
        raise RuntimeError("no requested AMD smoke kernels were found")

    source = Path(__file__).resolve()
    hsaco_source = source.parent / "flagos_amd_hsaco.py"
    (args.output_dir / "manifest.json").write_text(json.dumps({
        ...
        "package": {
            ...
            "source_sha256": hashlib.sha256(
                source.read_bytes() + b"\0" + hsaco_source.read_bytes()).hexdigest(),
            "sources": [
                {"name": source.name, "sha256": hashlib.sha256(source.read_bytes()).hexdigest()},
                {"name": hsaco_source.name,
                 "sha256": hashlib.sha256(hsaco_source.read_bytes()).hexdigest()},
            ],
        },
        "kernels": entries,
    }, indent=2) + "\n")
    ...
    print(f"wrote {len(entries)} kernels for {target.backend}:{target.arch} to {final_output_dir}")
