...
int main() {
    ...
    CHECK(flagos_device_caps_are_valid(&caps));
    for (const flagos_provider_kind kind : {
            flagos_provider_kind::ai_accelerator,
            flagos_provider_kind::cpu_accelerator,
            flagos_provider_kind::dsp,
            flagos_provider_kind::matrix,
            flagos_provider_kind::other,
        }) {
        caps.kind = kind;
        caps.memory = FLAGOS_MEMORY_DEVICE_LOCAL;
        CHECK(flagos_device_caps_are_valid(&caps));
    }
    caps.memory = 0;
    CHECK(!flagos_device_caps_are_valid(&caps));
