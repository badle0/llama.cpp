...
static const void * flagos_resolve_plan_data(void * user_data, const ggml_tensor * tensor) {
    const auto * context = static_cast<const ggml_backend_flagos_context *>(user_data);
    return context == nullptr || tensor == nullptr ? nullptr : flagos_resolve_data(context, tensor);
}

static bool flagos_storage_overlaps(
        const void * left_data, size_t left_size,
        const void * right_data, size_t right_size) {
    if (left_data == nullptr || right_data == nullptr || left_size == 0 || right_size == 0) {
        return true;
    }
    const uintptr_t left = reinterpret_cast<uintptr_t>(left_data);
    const uintptr_t right = reinterpret_cast<uintptr_t>(right_data);
    if (left > UINTPTR_MAX - left_size || right > UINTPTR_MAX - right_size) {
        return true;
    }
...
static flagos_lowering_choice flagos_denglin_query_lowering(
    ...
    if (context == nullptr || cgraph == nullptr || !context->fusion_enabled ||
    ...
    // This legacy AOT symbol writes both outputs and has no alias-specialized
    // ABI. Keep its contract as strict as the common plan allows: all output
    // ranges are disjoint from one another and from both inputs. In
    // particular, GGML may reuse the dead RMSNorm allocation for MUL; that
    // layout must execute as two direct operations on Denglin instead of
    // inheriting AMD's separate in-place lowering.
    const void * input_data = flagos_resolve_data(context, norm->src[0]);
    const void * weight_data = flagos_resolve_data(context, weight);
    const size_t output_size = ggml_nbytes(norm);
    if (candidate.required_output_node_indices.empty() ||
        candidate.required_output_node_indices.back() != candidate.node_indices.back() ||
        flagos_storage_overlaps(norm->data, output_size, mul->data, ggml_nbytes(mul)) ||
        flagos_storage_overlaps(norm->data, output_size, input_data, ggml_nbytes(norm->src[0])) ||
        flagos_storage_overlaps(norm->data, output_size, weight_data, ggml_nbytes(weight)) ||
        flagos_storage_overlaps(mul->data, ggml_nbytes(mul), input_data, ggml_nbytes(norm->src[0])) ||
        flagos_storage_overlaps(mul->data, ggml_nbytes(mul), weight_data, ggml_nbytes(weight))) {
        return choice;
    }
...
static enum ggml_status flagos_graph_evaluate(
    ...
    for (const auto & step : plan.steps) {
        ...
        if (step.kind == flagos_execution_kind::pattern) {
            launched = flagos_execute_fusion_step(fusion_interface, cgraph, step);
        } else {
            switch (node->op) {
            case GGML_OP_NONE:
...
static enum ggml_status ggml_backend_flagos_graph_compute(
    ...
    const flagos_fusion_interface fusion_interface {
        /* .query_lowering    = */ flagos_denglin_query_lowering,
        /* .validate_lowering = */ flagos_denglin_query_lowering,
        /* .execute_fusion    = */ flagos_denglin_execute_fusion,
        /* .user_data         = */ context,
        /* .configuration_id  = */ context->fusion_enabled ? 1ULL : 0ULL,
        /* .resolve_tensor_data = */ flagos_resolve_plan_data,
    };
    ...
    if (!context->graph_capture_enabled || !plan.capture_safe() || cgraph->n_nodes < 16) {
        return flagos_graph_evaluate(context, cgraph, plan, fusion_interface);
    }
    ...
    const enum ggml_status status = flagos_graph_evaluate(
        context, cgraph, plan, fusion_interface);
